...
if (!$hosted_zone) {
    ...
    if ($change->status eq 'INSYNC') {
        is($change->status, 'INSYNC',
            "I created a new hosted zone " . $hosted_zone->name);
        lives_ok {
            ($hosted_zone) = $route53->get_hosted_zones($fq_test_domain);
        }
    ...
    if (scalar(@$current_records)) {
        ...
        my $first_deletion =
            $route53->atomic_update(\@first_deletes,
        ...
        if (@$leftover_records > 2) {
            ...
            my ($soa_record) = grep {$_->type eq 'SOA'} @$current_records;
            my $second_deletion =
                $route53->atomic_update($leftover_records,
                [ $ns_record, $soa_record ], 1);
...
is($second_creation->status, 'INSYNC', "I saved my last 1 record to Route53");
...
ok(defined($hosted_zone), "And I can get my hosted zone " . $hosted_zone->name);
my $rrsets;
...
my $first_deletion =
    $route53->atomic_update(\@first_deletes, [ $ns_record, $soa_record ], 1);
ok($first_deletion->status =~ /INSYNC|NOOP/,
